...
> **What this file is.** The single source of truth for building the Unchained mobile app: brand, design system, screen specs, and technical architecture. It sits alongside the two research documents in this folder:
> - `Unchained_Product_Research_and_Design_Documentation-1.docx` - the *why* (behavioral science, UX doctrine, personas).
> - `Unchained_PH_Market_Launch_Plan.docx` - the *go-to-market* (the Tarsi-inspired Philippine launch).
>
...
**Unchained** helps anyone quit or cut back on a compulsive behavior - vaping, smoking, online gambling (*sugal*), alcohol, pornography, social media, gaming, shopping, or a custom habit. Unlike single-habit trackers, Unchained treats recovery as one shared problem: the same craving loop underneath, the same tools on top.

Three ideas carry the whole product:

1. **The craving moment is the product.** Every competitor is built around a streak counter. Unchained is built around the 90-180 seconds when a user is actively fighting an urge - a full-screen **SOS flow** reachable in one tap from anywhere.
2. **Shame is what makes people quit the app.** A relapse must never reset progress to zero. Unchained uses **Recovery Strength** (0-100) that dips gently and rebuilds - lifetime effort is permanent.
...
**The one-line positioning** (from the launch plan): *For Filipinos who want to quit or cut back but relapse when cravings hit, Unchained intervenes at the craving moment and tracks progress without shame - unlike trackers that punish a slip with a reset to zero. Built in the Philippines by DotOrbit.*
...
The assets in `/assets` show a **lamb breaking free of chains** - the core promise behind *Unchained*. A sheep that was bound and is now getting loose. It is soft, small, and brave: exactly the tone the research asks for ("the steady friend who's been there - calm, honest, quietly strong, warm without saccharine").

**Name: "Unchained."** The mascot carries the same name as the app: a small, brave companion that makes the brand feel alive without adding another thing for the user to remember.
...
2. **The sheep never drives re-engagement.** No "Unchained misses you" notifications. Guilt-based re-engagement is banned (research §14, launch plan §5.2).
...
│
├── TAB 1 · TODAY (home)
│   ├── Unchained (mascot, state-driven) + greeting
...
4. **Commitment ritual** - hold a button 3 seconds to *"help Unchained break the first link."* Effortful-on-purpose; creates a memory anchor (Cialdini commitment/consistency).
...
│              (Unchained waves, happy) │  MascotView, idle-happy
│        ╭────────────╮                │  StrengthRing 168pt
...
**Zero red. Zero broken-chain-shattering imagery. Unchained sits beside the user (comfort state), briefly.**
...
[I slipped]  (always available, never buried)
   ...
   │   (Unchained, comfort state, beside the user) [Continue]
...
| Re-engagement | 1 after 48h, 1 after 7d, then stop | usual active hour | compassion, **never guilt** ("Unchained misses you" is banned) |
...
## 13. Ethical monetization (Unchained Plus)
...
4. **The user is the author.** Unchained suggests; the user decides. Autonomy is never traded for engagement.
...
*Prepared for DotOrbit · Unchained mobile app · React Native (Expo) · Clean Architecture. Palette sampled from `/assets`; behavioral doctrine inherited from the product research; launch inspiration from Tarsi by Bryl Lim.*
