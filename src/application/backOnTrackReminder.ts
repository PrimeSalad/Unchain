/**
 * Back on Track - weekly reminder notification.
 *
...
const CHANNEL_ID = 'back-on-track-reminders';
const NOTIF_TAG = 'back-on-track-reminder';
...
/**
 * Schedule a weekly reminder notification that fires when the next Back on
 * Track assessment becomes available. Cancels any existing reminders first.
 */
export async function scheduleBackOnTrackReminder(
  ...
  try {
    ...
    await cancelBackOnTrackReminders();

    const nextAvailableAt = lastCompletedAt != null
      ? lastCompletedAt + BACK_ON_TRACK_INTERVAL_MS
      : Date.now() + 60_000;
    ...
    await notifs.scheduleNotificationAsync({
      content: {
        ...
        data: {
          tag: NOTIF_TAG,
          deepLink: 'unchainly://back-on-track',
        },
      },
      trigger: {
...
/**
 * Cancel all pending Back on Track reminder notifications.
 */
export async function cancelBackOnTrackReminders(): Promise<void> {
  const notifs = await getNotifs();
  if (!notifs) return;
  try {
