/**
 * Press Pause - the single hub screen.
 * Route: /press-pause-log
 *
 * Stats, trend charts, expandable entry cards, add button, insights.
 * Everything Press Pause lives here. After submitting the assessment
 * (via /press-pause), the user lands back here with the new entry visible.
 */
...
import {
  pressPauseAvailability,
  generateInsights,
  randomSupportiveMessage,
  type PressPauseEntry,
...
function EntryCard({ entry, index }: { entry: PressPauseEntry; index: number }) {
  const theme = useTheme();
  ...
  const rows: { icon: string; color: string; label: string; value: string }[] = [
    { icon: 'time-outline', color: theme.color.primary, label: 'Satisfaction', value: entry.timeSatisfaction },
    { icon: 'game-controller-outline', color: theme.color.primary, label: 'Gaming Time', value: entry.gamingTime },
    { icon: 'alert-circle-outline', color: entry.interference === 'No' ? theme.color.success : theme.color.danger, label: 'Interference', value: entry.interference },
    { icon: 'walk-outline', color: entry.physicalActivity === 'Yes' ? theme.color.success : theme.color.danger, label: 'Physical', value: entry.physicalActivity },
    { icon: 'moon-outline', color: entry.sleepQuality.includes('Good') || entry.sleepQuality.includes('Excellent') ? theme.color.success : theme.color.danger, label: 'Sleep', value: entry.sleepQuality },
    ...
    { icon: 'balance-outline', color: entry.balanceLevel.includes('More') ? theme.color.success : theme.color.danger, label: 'Balance', value: entry.balanceLevel },
  ];
        ...
        <View style={{
          ...
          <View style={{
            ...
            <View style={{
              ...
              <Ionicons name="pause" size={18} color={theme.color.primary} />
            </View>
            <View style={{ flex: 1 }}>
              ...
              <Text variant="caption" dim style={{ marginTop: 1 }}>
                Balance: {entry.balanceLevel} · Sleep: {entry.sleepQuality}
              </Text>
            </View>
...
export default function PressPauseLogScreen() {
  const router = useRouter();
  const theme = useTheme();
  const back = useSafeBack();
  const [deletingId, setDeletingId] = useState<string | null>(null);

  const pressPauseEntries = useStore((s) => s.pressPauseEntries);
  const deletePressPauseEntry = useStore((s) => s.deletePressPauseEntry);

  const sortedEntries = useMemo(
    () => [...pressPauseEntries].sort((a, b) => b.at - a.at),
    [pressPauseEntries],
  );

  const availability = useMemo(
    () => pressPauseAvailability(sortedEntries[0]?.at ?? null),
    [sortedEntries],
  );
  ...
  const weeksBalanced = useMemo(() => {
    let count = 0;
    for (const entry of sortedEntries) {
      if (entry.balanceLevel.includes('More')) count++;
      else break;
    }
    return count;
  }, [sortedEntries]);

  const totalReflections = pressPauseEntries.length;
  ...
  const confirmDelete = useCallback(() => {
    if (deletingId) {
      deletePressPauseEntry(deletingId);
      setDeletingId(null);
    }
  }, [deletingId, deletePressPauseEntry]);
  ...
  return (
    <Screen background={theme.color.bg}>
      ...
      <View style={{
        ...
        <Text variant="headline" style={{ flex: 1, fontFamily: 'Nunito_900Black' }}>Press Pause</Text>
      </View>
      ...
      >
        ...
        <View style={{ flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.lg }}>
          <StatCard icon="calendar" value={weeksBalanced} label="Weeks Balanced" color={theme.color.success} delay={200} />
        ...
        {availability.available ? (
          <Pressable
            onPress={() => router.push('/press-pause')}
            style={({ pressed }) => ({
        ...
        {sortedEntries.length === 0 ? (
          <View style={{
            ...
            <Ionicons name="pause-outline" size={32} color={theme.color.textDim} />
            <Text variant="footnote" dim style={{ textAlign: 'center', lineHeight: 20 }}>
              Complete your first reflection to start tracking your balance over time.
            </Text>
          </View>
        ) : (
          <View style={{ gap: spacing.sm }}>
        ...
        <Text variant="caption" dim style={{ marginTop: spacing.xl, textAlign: 'center', lineHeight: 16 }}>
          Press Pause is a self-reflection tool for monitoring gaming balance. It is not a clinical assessment or diagnosis.
        </Text>
      </Animated.ScrollView>
