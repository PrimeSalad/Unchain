/**
 * Catch Your Breath - History Log
 * Shows all past weekly lung health assessments.
 * Route: /catch-your-breath-history
 */
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
        <View style={{
          ...
          <Ionicons name="fitness" size={20} color={theme.color.success} />
        </View>
        <View style={{ flex: 1 }}>
...
function DetailSheet({
  ...
  entry: CatchYourBreathEntry | null;
  visible: boolean;
  onClose: () => void;
  onDelete: () => void;
}) {
  ...
  const rows: { label: string; value: string }[] = [
    { label: 'Breathing', value: entry.breathingRating },
    { label: 'Comparison', value: entry.breathingComparison },
    { label: 'Coughing', value: entry.coughFrequency },
    { label: 'Mucus', value: entry.mucusFrequency },
    { label: 'Out of Breath', value: entry.shortnessOfBreath },
    { label: 'Wheezing', value: entry.wheezingFrequency },
    { label: 'Chest', value: entry.chestDiscomfort },
    { label: 'Activity', value: entry.activityTolerance },
    { label: 'Smoking', value: entry.smokingFrequency },
  ];
      ...
      <View style={{ gap: spacing.md }}>
        <View style={{ alignItems: 'center', gap: spacing.sm }}>
          <View style={{
            ...
            <Ionicons name="fitness" size={28} color={theme.color.success} />
          </View>
...
export default function CatchYourBreathHistory() {
  const theme = useTheme();
  const safeBack = useSafeBack();
  const router = useRouter();
  const entries = useStore((s) => s.catchYourBreathEntries);
  const deleteCatchYourBreathEntry = useStore((s) => s.deleteCatchYourBreathEntry);

  const [selectedEntry, setSelectedEntry] = useState<CatchYourBreathEntry | null>(null);
  const [sheetVisible, setSheetVisible] = useState(false);

  const openEntry = (entry: CatchYourBreathEntry) => {
  ...
  const deleteEntry = () => {
    if (selectedEntry) {
      deleteCatchYourBreathEntry(selectedEntry.id);
    }
  };
  ...
  return (
    <Screen edges={['top', 'bottom']}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md, marginTop: spacing.sm }}>
        ...
        <View style={{ flex: 1 }}>
          <Text variant="headline">Catch Your Breath History</Text>
      ...
      {entries.length === 0 ? (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.xl }}>
          ...
          <Text variant="footnote" dim center style={{ lineHeight: 20, marginBottom: spacing.lg }}>
            Complete your first Catch Your Breath reflection to start tracking your lung health over time.
          </Text>
