/**
 * Where Did It Go? - the single hub screen.
 * Route: /where-did-it-go-log
 *
 * Stats, trend charts, expandable entry cards, add button, insights.
 * Everything Where Did It Go? lives here. After submitting the assessment
 * (via /where-did-it-go), the user lands back here with the new entry visible.
 */
...
import {
  whereDidItGoAvailability,
  generateInsights,
  randomSupportiveMessage,
  type WhereDidItGoEntry,
...
function EntryCard({ entry, index }: { entry: WhereDidItGoEntry; index: number }) {
  const theme = useTheme();
  ...
  const rows: { icon: string; color: string; label: string; value: string }[] = [
    { icon: 'wallet-outline', color: theme.color.primary, label: 'Confidence', value: entry.financialConfidence },
    { icon: 'flame-outline', color: entry.gamblingUrges === 'None' ? theme.color.success : theme.color.danger, label: 'Urges', value: entry.gamblingUrges },
    { icon: 'close-circle-outline', color: entry.didYouGamble === 'No' ? theme.color.success : theme.color.danger, label: 'Gambling', value: entry.didYouGamble },
    { icon: 'cart-outline', color: entry.spendingSatisfaction.includes('Satisfied') ? theme.color.success : theme.color.danger, label: 'Satisfaction', value: entry.spendingSatisfaction },
    { icon: 'cash-outline', color: entry.didYouSave === 'Yes' ? theme.color.success : theme.color.danger, label: 'Saved', value: entry.didYouSave },
    { icon: 'heart-outline', color: entry.meaningfulSpending === 'Yes' ? theme.color.success : theme.color.danger, label: 'Meaningful', value: entry.meaningfulSpending },
    { icon: 'trending-up-outline', color: entry.spendingControl.includes('More') ? theme.color.success : theme.color.danger, label: 'Control', value: entry.spendingControl },
  ];
        ...
        <View style={{
          ...
          <View style={{
            ...
            <View style={{
              ...
              <Ionicons name="wallet" size={18} color={theme.color.primary} />
            </View>
            <View style={{ flex: 1 }}>
              ...
              <Text variant="caption" dim style={{ marginTop: 1 }}>
                Confidence: {entry.financialConfidence} · Urges: {entry.gamblingUrges}
              </Text>
            </View>
...
export default function WhereDidItGoLogScreen() {
  const router = useRouter();
  const theme = useTheme();
  const back = useSafeBack();
  const [deletingId, setDeletingId] = useState<string | null>(null);

  const whereDidItGoEntries = useStore((s) => s.whereDidItGoEntries);
  const deleteWhereDidItGoEntry = useStore((s) => s.deleteWhereDidItGoEntry);

  const sortedEntries = useMemo(
    () => [...whereDidItGoEntries].sort((a, b) => b.at - a.at),
    [whereDidItGoEntries],
  );

  const availability = useMemo(
    () => whereDidItGoAvailability(sortedEntries[0]?.at ?? null),
    [sortedEntries],
  );
  ...
  const weeksFree = useMemo(() => {
    ...
    for (const entry of sortedEntries) {
      if (entry.didYouGamble === 'No') count++;
      else break;
    }
    return count;
  }, [sortedEntries]);

  const totalReflections = whereDidItGoEntries.length;
  ...
  const confirmDelete = useCallback(() => {
    if (deletingId) {
      deleteWhereDidItGoEntry(deletingId);
      setDeletingId(null);
    }
  }, [deletingId, deleteWhereDidItGoEntry]);
  ...
  return (
    <Screen background={theme.color.bg}>
      ...
      <View style={{
        ...
        <Text variant="headline" style={{ flex: 1, fontFamily: 'Nunito_900Black' }}>Where Did It Go?</Text>
      </View>
      ...
      >
        ...
        <View style={{ flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.lg }}>
          <StatCard icon="calendar" value={weeksFree} label="Weeks Free" color={theme.color.success} delay={200} />
        ...
        {availability.available ? (
          <Pressable
            onPress={() => router.push('/where-did-it-go')}
            style={({ pressed }) => ({
        ...
        {sortedEntries.length === 0 ? (
          <View style={{
            ...
            <Text variant="footnote" dim style={{ textAlign: 'center', lineHeight: 20 }}>
              Complete your first reflection to start tracking your financial habits over time.
            </Text>
          </View>
        ) : (
          <View style={{ gap: spacing.sm }}>
        ...
        <Text variant="caption" dim style={{ marginTop: spacing.xl, textAlign: 'center', lineHeight: 16 }}>
          Where Did It Go? is a self-reflection tool for monitoring financial habits. It is not a financial advisor or budget tool.
        </Text>
      </Animated.ScrollView>
